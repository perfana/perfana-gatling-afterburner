        ...
        stage('Checkout') {
            ...
            steps {
                ...
                script {
                    ...
                    git url: params.gatlingRepo, branch: 'java-client-1.5.0'
            ...
            steps {
                ...
                script {
                    ...
                    withCredentials([string(credentialsId: 'perfanaApiKey', variable: 'TOKEN')]) {
                        ...
                        sh """
                           ${mvnHome}/bin/mvn clean install -U -X events-gatling:test -Ptest-env-demo,${params.workload},assert-results -DtestRunId=${testRunId} -DbuildResultsUrl=${buildUrl} -Dversion=${version} -DsystemUnderTest=${system_under_test} -Dannotations="${params.annotations}" -DtargetBaseUrl=${targetBaseUrl} ${kubernetes}
                        """
                    }
