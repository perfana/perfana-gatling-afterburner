        ...
        stage('Checkout') {
            ...
            steps {
                ...
                script {
                    ...
                    git url: params.gatlingRepo, branch: "OptimusPrime-logstash"
                  ...
                  logstash {
                    script {
                        ...
                        withCredentials([string(credentialsId: 'perfanaApiKey', variable: 'TOKEN')]) {
                            ...
                            sh """
                               ${mvnHome}/bin/mvn clean -U events-gatling:test -Ptest-env-demo,${params.workload},assert-results -DtestRunId=${testRunId} -DbuildResultsUrl=${buildUrl} -Dversion=${version} -DsystemUnderTest=${system_under_test} -Dannotations="${params.annotations}" -DapiKey=$TOKEN -DtargetBaseUrl=${targetBaseUrl} ${kubernetes}
                            """
                        }
